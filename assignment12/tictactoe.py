...
def play_tictactoe():
    try:
        print(board.whats_next())
        while board.whats_next()[0] != True:
            print(''' Valid moves["upper left", "upper center", "upper right", "middle left", 
            "center", "middle right", "lower left", "lower center", "lower right"]
            ''')
            board.move(input(f"{board.turn} move: "))
            print(board)
        print(f"Congrats! {board.whats_next()[1]}")
